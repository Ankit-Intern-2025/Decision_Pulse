...
const CardedChart = ({ chartHeading, chartCustomization }) => {
  return (
    <div className={`${chartCustomization.chartType === "textList" ? "bg-[#095458] " : "bg-white"} p-4 rounded-lg`}>
      <div className="flex justify-between items-center mb-2">
        {/* <img src={logoSrc} alt="Logo" className="w-8 h-8" /> */}
        <h3 className={`${chartCustomization.chartType === "textList" ? " text-[white]" : " "} text-lg font-semibold text-center w-full`}>
          {chartHeading}
        </h3>
          ...
          <ul className="w-[100%] m-auto p-6  ">
            {chartCustomization.textItems.map((item, index) => (
              <li key={index} className="flex items-start mb-3">
                <FaArrowRight className="w-5 h-5 mr-3 mt-1 text-white" />
                <span className="text-white text-sm inline-block">{item}</span>
              </li>
            ))}
          </ul>
        </>
