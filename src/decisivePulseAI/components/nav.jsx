...
import { logout, rehydrate, setModules } from "../../store/authSlice"; // Rehydrate action
import { fetchModules } from "../../http/dashboard_api";
...
const Main = ({title}) => {
  ...
  const {setLoadingModules, setModuleErrMsg} = UseContext()
  ...
  const handleResearch =() =>{
    navigate("/secondary-research", { replace: true });
  }
    ...
    const getModules = async ()=>{
      ...
      try{
        if(response.length>0){
          localStorage.setItem("modules_list", JSON.stringify(response))
          dispatch(setModules(response))
          setModuleErrMsg("")
        }else{
          setModuleErrMsg("You are not assigned to any dashboards, please contact your administrator.")
        }
      }catch(err){
        setModuleErrMsg("Unable to get any dashboards, please contact your administrator.")
        console.log(err)
      }finally{
      ...
      >
        ...
        <div className="gap-6 flex items-center">
          ...
          {isLoggedIn && <button
            ...
            className="text-white bg-[#00acc1] p-2 self-center  hover:#19746b rounded-[50%]"
          >
          ...
          {isLoggedIn && <button
            ...
            className="text-white bg-[#00acc1] p-2 self-center  hover:#19746b rounded-[50%]"
          >
            <img src={researchimage}  title="Secondary Research" alt="Research Image" />
          </button>}
        </div>
      </div>
